package com.example.flamemathnew.back.support
...
//создаём массив с n элементов, создаваемых при помощи лямбды. Первый аргумент - лямба, второй аргумент - количество
        inline fun <reified T> createSingleArrayList(func: () -> T, n: Int): MutableList<T> {
        ...
        inline fun <reified T> createRectangleArrayList(
            func: () -> T,
            m: Int,
            n: Int
        ): MutableList<MutableList<T>> {
            val array: MutableList<MutableList<T>> = mutableListOf()
            if (n > 0) {
                var i: Int = 0;
