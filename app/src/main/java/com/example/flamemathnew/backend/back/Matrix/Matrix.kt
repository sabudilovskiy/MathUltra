package com.example.flamemathnew.backend.back.Matrix
...
import com.example.flamemathnew.backend.mid.Computer.DEGENERATE_MATRIX
import com.example.flamemathnew.backend.mid.Computer.INVALID_NUMBER_STRING
import com.example.flamemathnew.backend.mid.Computer.MATRIX_DIMENSION_MISSMATCH
import com.example.flamemathnew.backend.mid.Computer.NON_QUADRATIC_MATRIX
import com.example.flamemathnew.backend.back.Numbers.DecNumb
import com.example.flamemathnew.backend.back.Numbers.FractionalNumb
import com.example.flamemathnew.backend.back.Numbers.Numb
...
import com.example.flamemathnew.backend.back.Parameters.Inverse.*
import Parameters.Rank
import com.example.flamemathnew.backend.back.Support.CTI
import com.example.flamemathnew.backend.mid.Settings.matrix
...
open class Matrix : Ring {
    ...
    public fun determinant(): Ring {
        if (m == n) {
            try {
                val cur_method: Det = matrix.Det.getCurMethod(m);
    ...
    fun det_with_saruss_rule(): Ring {
        return if (m == n && m == 3) {
            ...
            commit("det = a${CTI(11)}*a${CTI(22)}*a${CTI(33)} + a${CTI(12)}*a${CTI(23)}*a${CTI(31)} + a${CTI(13)}*a${CTI(21)}*a${CTI(32)} - a${CTI(31)}*a${
                CTI(22)
            }*a${CTI(13)} - a${CTI(21)}*a${CTI(12)}*a${CTI(33)} - a${CTI(11)}*a${CTI(32)}*a${CTI(23)} =", METHOD_RULES)
            commit(
