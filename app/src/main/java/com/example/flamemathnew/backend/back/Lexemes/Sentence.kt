...
import com.example.flamemathnew.backend.back.Lexemes.ErrorHandler.setError
...
class Sentence {
            ...
            if (pos < input.length) {
                ...
                if (Support.is_numeral(input[pos])) {
                    ...
                    if (max_id == -1) {
                        ...
                        val id = Id_lexemes.getById(max_id)
                        var temp: Lexeme
                        pos = pos_max
    ...
    fun find_highest_priority(): Int {
        ...
        while (i < _array.size) {
            if (_array[i].get_id() !== Id_lexemes.ARGUMENT) {
                val cur_priority = Archieve.getPriority(_array[i].get_id())
    ...
    fun find_countable_operator(priority: Int): Int {
        var i = 0
        while (i < _array.size) {
            if (_array[i].get_id() !== Id_lexemes.ARGUMENT && Archieve.getPriority(_array[i].get_id()) == priority) {
                val left = Archieve.getLeftArgue(_array[i].get_id())
                val right = Archieve.getRightArgue(_array[i].get_id())
    ...
    fun have_errors(): Boolean {
        val n = _array.size
        if (Archieve.getLeftArgue(_array[0].get_id()) == 0 && Archieve.getRightArgue(_array[n-1].get_id()) == 0) {
            for (i in 1 until n) {
                val cur_id = _array[i].get_id()
                val left = Archieve.getLeftArgue(cur_id)
                val right = Archieve.getRightArgue(cur_id)
                if (left > 0 && right > 0) {
                    if (Archieve.getRightArgue(_array[i - 1].get_id()) == 0 && Archieve.getLeftArgue(_array[i + 1].get_id()) == 0) {
                    } else {
                    ...
                    if (Archieve.getLeftArgue(_array[i + 1].get_id()) == 0 && Archieve.getRightArgue(_array[i - 1].get_id()) != 0) {
                    } else {
    ...
    fun count(): Lexeme {
        ...
        while (a != 0) {
            ...
            while (b != -1) {
                val left = Archieve.getLeftArgue(_array[b].get_id())
                val right = Archieve.getRightArgue(_array[b].get_id())
                ...
                if (Archieve.checkCountable(_array[b].get_id(), argue)) {
                    val x0 = ArrayList<Double>()
